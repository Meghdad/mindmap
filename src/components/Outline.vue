...
export default {
  ...
  methods: {
    ...
    drawOutline(dJSON) {
      const { outlineGNode, outlineGPath } = this
      ...
      function draw(r) {
        ...
        outlineGNode.selectAll('g')
          .data(rDescendants)
          .join(
            (enter) => appendNode(enter),
            (update) => updateNode(update),
          )
        outlineGPath.selectAll('path')
          .data(r.links())
          .join(
  ...
  mounted() {
    ...
    this.outlineGNode = this.outline_svg.append('g').attr('class', 'outnodes')
    this.outlineGPath = this.outline_svg.append('g').attr('class', 'outpath')
  },
}
</script>
